...
# works with specific values not init checksum, len
# lls hdr checksum set to zero when auth enabled
 
# optimize for diff packet types
...
import struct
from hashlib import md5  #edit
from scapy.packet import bind_layers, Packet
from scapy.fields import BitField, BitFieldLenField, ByteEnumField, ByteField, \
...
class OSPFLLSOptionsField(FlagsField):
...
class OSPF_Hdr(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(BitFieldLenField("authdatalen", 16, 8, length_of="authdata16"), lambda pkt: pkt.authtype == 2),  # noqa: E501
        ConditionalField(XIntField("seq", 0), lambda pkt: pkt.authtype == 2),
        ConditionalField(StrLenField("authdata16", b"", length_from=lambda x : x.authdatalen), lambda pkt : pkt.authtype == 2) # noqa: E501
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            if self.authtype == 2:
                # insert  checksum value of 0
                ck = 0   # Crypto, see RFC 2328, D.4.3
                p = p[:12] + struct.pack("!H", ck) + p[14:]

                authdatalen = len(self.authdata16) #get actual length
                p = p[:24] + p[24+authdatalen:] #strip authdata

                ospf_data_offset = int.from_bytes(p[2:4], signed=False)
 
                if authdatalen < 16:
                    #pad end with zero bytes
                    self.authdata16 += b"\x00" * (16 - authdatalen)
                else:
                    #truncate to 16 bytes
                    self.authdata16 = self.authdata16[:16]
                
                bytes_to_hash = p[:ospf_data_offset] + self.authdata16
                md5_digest = md5(bytes_to_hash).digest()
                p = p[:ospf_data_offset] + md5_digest + p[ospf_data_offset:]
                if self.type == 1:
                    #hello packet 24 bytes
                    if p[30] & 0x10 == 0x10: #check l bit set in hello packet or dd packet
                        #LLS bit is set
                        #Compute authentication digest for LLS block
                        lls_offset = ospf_data_offset + 16 
                        lls_payload = p[lls_offset:]

                        #Alter checksum
                        lls_payload = struct.pack("!H", 0) + lls_payload[2:]
                        # TODO: check if Crypto Authentication TLV is added to LLS block
                        lls_payload = lls_payload[:-20] + (self.seq if isinstance(self.seq, bytes) else struct.pack("!I", self.seq))
                        #Set Crypto Authentication TLV Sequence No. to OSPF Header's sequence number
                            #strip Crypto Authentication TLV authentication data
                        lls_bytes = lls_payload + self.authdata16
                        lls_md5_digest = md5(lls_bytes).digest()
                        p = p[:lls_offset] + lls_payload + lls_md5_digest
                ...
                p = p[:12] + struct.pack("!H", ck) + p[14:]
                    ...
                    # Check Crypto Authentication TLV exists, if yes strip
...
class LLS_Extended_Options(LLS_Generic_TLV):
    ...
    fields_desc = [ShortField("type", 1),
                   FieldLenField("len", 4, fmt="!H", length_of="options"),
                   OSPFLLSOptionsField()]
...
class LLS_Crypto_Auth(LLS_Generic_TLV):
    ...
    fields_desc = [ShortField("type", 2),
                   FieldLenField("len", 20, fmt="H", length_of=lambda x: x.authdata + 4),  # noqa: E501
                   XIntField("sequence", 0),
                   StrLenField("authdata", b"\x00" * 16, length_from=lambda x: x.len - 4)]  # noqa: E501
 
class LLS_Local_Interface_ID(LLS_Generic_TLV):
    name = "LLS Local Interface ID"
    fields_desc = [ShortField("type", 18),
                   FieldLenField("len", None, length_of="val"),
                   StrLenField("val", "", length_from=lambda x: x.len)]
